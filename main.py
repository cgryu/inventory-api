...
def read_item(item_id: int, conn = Depends(get_conn)):
...
def list_items(limit: int = 10, skip: int = 0, conn = Depends(get_conn)):
    with conn.cursor() as cur:
        cur.execute(
...
def create_item(item: Item, conn = Depends(get_conn)):
    try:
        with conn.cursor() as cur:
            cur.execute(
...
def update_item(item_id: int, item: Item, conn = Depends(get_conn)):
    try:
        with conn.cursor() as cur:
            cur.execute(
...
def patch_item(item_id: int, update: ItemUpdate, conn = Depends(get_conn)):
    with conn.cursor() as cur:
        cur.execute(
            "SELECT item_id, name, price, in_stock FROM items WHERE item_id=%s",
            (item_id,)
