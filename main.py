...
def read_item(item_id: int):
    ...
    return to_output(items_db[item_id])

@app.get("/items", response_model=list[ItemOut])
def list_items(limit: int = 10, skip: int = 0):
    return [to_output(item) for item in list(items_db.values())[skip:skip+limit]]
...
def create_item(item: Item):
    ...
    oldID = itemID
    itemID += 1
    return to_output(items_db[oldID])
